 ...
 * @author root
 */
public class FingerMatchIsoToAnsi {
...
//            System.out.println("data length from verify is "+dataFromDB.length);
       ...
       /* System.out.println("cust_no "+cust_no+" will now be verified");
        try(PrintWriter out = new PrintWriter("/u01/status.txt")){
            out.println("cust_no "+cust_no+" will now be verified");
        }catch(Exception e){
        }
        
        try{ 
            FileOutputStream  fos1 = new FileOutputStream("varifyData.txt");
            fos1.write(varifyData);
            fos1.close();
            FileOutputStream  fos2 = new FileOutputStream("dataFromDB.txt"); 
            fos2.write(dataFromDB);
            fos2.close();
        }catch(Exception e){
        } */

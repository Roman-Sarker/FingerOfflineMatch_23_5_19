...
public class StandaredFPData {
    ...
    public static void methodIsoFP() {
        ...
        HashMap<Integer, List<Integer>> mapAgentCustNo = receiveAgentCusNo(vStatus);
        ...
        try {
            ...
            System.out.println("---Total (" + vStatus + ") Customer : " + mapAgentCustNo.size());
            for (Map.Entry<Integer, List<Integer>> entry : mapAgentCustNo.entrySet()) { // This loop change next customer
                ...
                if (entry.getValue().size() >= 1) {         // This condition check agent are available or not for specific customer
                    System.out.println("=== Customer No : " + entry.getKey() + ", And his total agent & ARO is : " + entry.getValue().size() + "===");
                    ...
                    for (int i = 0; i < entry.getValue().size(); i++) { // This loop change next Agent of specific Customer
                        ...
                        while (rs.next()) {
                            ...
                            if (aLlittle != null) {
                                ...
                                agentAvaFPmap.put("LLITTLE", aRlittle);
                            }
                            if (aLring != null) {
...
//                                    for (int a = 0; a < agentAvaFP.size(); a++) {
                        ...
                        for (Map.Entry<String, byte[]> custEntry : custAvaFPmap.entrySet()) {
                            for (Map.Entry<String, byte[]> agentEntry : agentAvaFPmap.entrySet()) {
...
//                                        System.out.println(custAvaFP.get(c));
                                ...
                                byte[] bCustFP = custEntry.getValue();
                                byte[] bAgentFP = agentEntry.getValue();
...
//                                            System.out.println(custAvaFP.get(c));
//                                            System.out.println("CustFPNo = "+ CustFpName +"| AgentFpNo = "+ agentFpName);
//                                            System.out.println("Finger Matched");
                                    ...
                                    MatchFingerInsert.insertFinger(customerId, AgentId, agentFpName, CustFpName, vStatus);
            ...
            System.out.println("Exception raise in StandaredFPData class for Finger data not found. Error is : " + ex.getMessage());
            ex.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
